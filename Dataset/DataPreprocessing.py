...
from utils.Embedding import seq2kmer
...
class SampleReader:
    ...
    def get_seq(self, Test=False):
        ...
        model_path = r"./DNABERT"
        tokenizer = BertTokenizer.from_pretrained(model_path)
        completed_seqs = [tokenizer(seq2kmer(raw_sentence, k=k), return_tensors='pt')['input_ids'].squeeze(0) for raw_sentence in row_seq.loc[:, 1]]
        completed_seqs = torch.nn.utils.rnn.pad_sequence(completed_seqs, batch_first=True, padding_value=tokenizer.pad_token_id)
